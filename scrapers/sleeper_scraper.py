...
class SleeperScraper:
    ...
    VALID_POSITIONS = {'QB', 'RB', 'WR', 'TE', SLEEPER_POSITION_RDP}
    ...
    def parse_sleeper_players(players_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for sleeper_id, player_data in players_data.items():
            try:
                ...
                is_rdp = position == SLEEPER_POSITION_RDP

                if not player_data.get('active', False) and not is_rdp:
                    continue

                if position not in SleeperScraper.VALID_POSITIONS:
                    continue

                if not sleeper_api_dict_should_persist(player_data):
                    invalid_players += 1
                    continue
